...
public class CircleGrid extends GridPane {
    ...
    public void makeTrackMarblesSelectable() {
        ...
        for (CircleCellPair pair : circleCellPairs) {
            ...
            if (c.equals(Color.web("#fad082")) || c.getOpacity() <= 0.5) continue;
            Cell cell = pair.getCell();
            boolean isOwnMarble = c.equals(playerColor);
            // Opponent marbles on their own Base Cell are immune to Five
            if (!isOwnMarble && cell != null && cell.getCellType() == CellType.BASE
                    && cell.getMarble() != null) continue;
            makeCircleSelectable(pair);
        }
    }
    ...
    public void highlightLandingCell(int fromTrackIndex, int steps) {
        // Clear any previous landing highlight
        clearLandingHighlight();
        if (steps == 0) return;
        int target = (fromTrackIndex + steps + 100) % 100;
        if (target >= 0 && target < circleCellPairs.size()) {
            javafx.scene.shape.Circle c = circleCellPairs.get(target).getCircle();
            c.setStroke(javafx.scene.paint.Color.LIMEGREEN);
            c.setStrokeWidth(3.0);
        }
    }

    /** Clears the green landing-cell highlight. */
    public void clearLandingHighlight() {
        for (CircleCellPair pair : circleCellPairs) {
            if (!selectedMarbles.contains(pair)) {
                javafx.scene.shape.Circle c = pair.getCircle();
                if (javafx.scene.paint.Color.LIMEGREEN.equals(c.getStroke())) {
    ...
    public void applyBoardHighlights(boolean showTrapCells) {
        for (int i = 0; i < circleCellPairs.size(); i++) {
            CircleCellPair pair = circleCellPairs.get(i);
            Cell cell = pair.getCell();
            javafx.scene.shape.Circle circle = pair.getCircle();
            if (selectedMarbles.contains(pair)) continue;
            boolean isTrap = cell != null && cell.isTrap();
            if (showTrapCells && isTrap) {
    ...
    // Optional callback fired whenever the marble selection changes (used for path highlight)
    private Runnable onSelectionChanged = null;
    public void setOnSelectionChanged(Runnable r) { this.onSelectionChanged = r; }
    private void fireSelectionChanged() { if (onSelectionChanged != null) onSelectionChanged.run(); }
}